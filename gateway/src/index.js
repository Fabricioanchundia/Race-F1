...
io.on('connection', (socket) => {
  ...
  socket.on('player:join', async ({ name, color, team, circuit }) => {
    ...
    try {
      ...
      socket.emit('player:registered', { carId, sectorId: 1, vectorClock: clock, raceStarted: !!data.raceStarted, queued: !!data.queued, circuit: data.circuit, grid: data.grid || null });
      if (!data.queued) {
        await redisPub.publish('race:events', JSON.stringify({
